...
let unsubFilaContatos = null;
onAuthStateChanged(auth, async (user) => {
  if (!user) {
    if (unsubFilaContatos) { unsubFilaContatos(); unsubFilaContatos = null; }
    filaContatos = {};
    return bloquearPainelAdmin();
  }
  try {
    const permitido = await usuarioEhAdmin(user);
    if (permitido) {
      atualizarUsuarioLogadoAdmin(user);
      liberarPainelAdmin();
      if (!unsubFilaContatos) {
        unsubFilaContatos = onValue(ref(db, "filaContatos"), (snapshot) => {
...
window.avisarWhatsapp = async function (chaveFila, numeroInicial, posicao, quadra, horario) {
  ...
  const previsaoTexto = quadra && horario && quadra !== "-"
    ? `Quadra ${quadra}, previsão: ${horario}.`
    : "Acompanhe a chamada pelo aplicativo ou procure o fiscal.";
  const mensagem1 = `Condomínio Laranjeiras — Sua vez está chegando! ${previsaoTexto} Dirija-se às quadras.`;
  window.open(`https://wa.me/${n}?text=${encodeURIComponent(mensagem1)}`, "_blank");

  // Aviso automático para o #2
  setTimeout(() => {
    const segundo = fila[1];
    if (!segundo) return;
    const chave2 = filaKeys[1];
    const contato2 = chave2 ? filaContatos[chave2] : null;
    if (!inscricaoOptouWhatsapp(segundo, contato2)) return;
    const n2 = normalizarWhatsapp(obterWhatsappInscricao(segundo, contato2));
    if (!n2) return;
    const mensagem2 = `Condomínio Laranjeiras — Você é o #2 da fila. Fique por perto! Caso o primeiro não compareça em 10 minutos, você será chamado.`;
    window.open(`https://wa.me/${n2}?text=${encodeURIComponent(mensagem2)}`, "_blank");
  }, 2000);
};

window.pularPrimeiro = async function () {
  if (fila.length < 2) return avisoAdmin("Não há próximo na fila para chamar.");
  const primeiro = fila[0];
  const chave1 = filaKeys[0];
  const nomes = formatarNomesPareados(primeiro.jogadores);
  const ok = await confirmarAdmin(`Remover "${nomes}" da fila e chamar o próximo?`, "Pular #1");
  if (!ok) return;
  await Promise.allSettled([
    remove(ref(db, `fila/${chave1}`)),
    remove(ref(db, `filaContatos/${chave1}`))
  ]);

  // Avisa o novo #1 (era o #2) se tiver WhatsApp
  const novo1 = fila[1];
  const chave2 = filaKeys[1];
  const contato2 = chave2 ? filaContatos[chave2] : null;
  if (novo1 && inscricaoOptouWhatsapp(novo1, contato2)) {
    const n = normalizarWhatsapp(obterWhatsappInscricao(novo1, contato2));
    if (n) {
      const msg = `Condomínio Laranjeiras — Sua vez chegou! Você é agora o #1 da fila. Dirija-se às quadras imediatamente.`;
...
document.getElementById("btnToggleWhatsapp").addEventListener("click", () => {
  set(ref(db, "config/whatsappAvisoAtivo"), !whatsappAvisoAtivo).catch((e) => avisoAdmin("Erro: " + e));
});

document.getElementById("btnToggleInscricoes").addEventListener("click", () => {
  set(ref(db, "config/inscricoesOnlineAtivas"), !inscricoesOnlineAtivas).catch((e) => avisoAdmin("Erro: " + e));
});
...
function atualizarBotaoWhatsapp() {
  document.getElementById("btnToggleWhatsapp").classList.toggle("off", !whatsappAvisoAtivo);
}
function atualizarBotaoInscricoes() {
  document.getElementById("btnToggleInscricoes").classList.toggle("off", !inscricoesOnlineAtivas);
}
...
function renderQuadras() {
  ...
  container.innerHTML = ativas.map((q) => {
    ...
    return `<div class="court-card ${estadoClasse}">
      ...
      <div id="timer-q${q.id}" class="court-timer${quaseAcabando || jogoEncerrado ? " warning" : ""}">${formatarTimer(q.tempoRestante || 0)}</div>
...
function renderFila(animar = false) {
  ...
  fila.forEach((j, idx) => {
    ...
    const btnPular = idx === 0 && fila.length > 1 ? `<button class="btn-pular-q" onclick="pularPrimeiro()" title="Pular #1"><i class="ri-skip-forward-line"></i></button>` : "";
